...
export const createCharacter = async (req, res) => {
  ...
  try {
    if (name === undefined || name === "")
      return res
        .status(400)
        .json({ Message: "El Nombre no puede estar vacio." });
    if (ki === undefined || ki === "")
      return res.status(400).json({ Message: "El Ki no puede estar vacio." });
    if (race === undefined || race === "")
      return res.status(400).json({ Message: "La Race no puede estar vacio." });
    if (gender === undefined || gender === "")
      return res
        .status(400)
        .json({ Message: "El Gender no puede estar vacio." });

    const kiEntero = Math.floor(ki);
    if (ki !== kiEntero)
      return res.status(400).json({ Message: "El ki tiene que ser entero." });

    if (gender !== "Female" && gender !== "Male")
      return res.status(400).json({ Message: "Gender debe ser Female o Male" });

    if (typeof description !== "string")
      return res
        .status(400)
        .json({ Message: "La description debe ser en string" });
    ...
    res
      .status(201)
      .json({ Message: "El personaje se ha creado con exito." }, character);
  } catch (error) {
  ...
  try {
    ...
    if (gender) {
      if (gender !== "Female" && gender !== "Male")
        return res
          .status(400)
          .json({ Message: "Gender debe ser Female o Male" });
    }
...
export const obtenerTodosLosPersonajes = async (req, res) => {
  try {
    ...
    if (personajes.length === 0)
      return res.json({ Message: "No hay nada en la base de datos" });
...
export const deleteCharacter = async (req, res) => {
  try {
    ...
    if (deleted > 0) {
      return res.status(200).json({ Message: "El personaje fue eliminado" });
    } else {
